  ...
  return (
    <div className={`absolute inset-0 z-40 flex flex-col bg-bg ${closing ? "slide-out-left" : "slide-in-left"}`} role="dialog" aria-label="Your conversations">
      ...
      <main className="no-scrollbar flex-1 overflow-y-auto px-5 pb-10">
        {/* Identity — you arrived here by tapping your own face */}
        <section className="rise mt-2 flex items-center gap-3 rounded-[16px] border border-border bg-bg-elev p-3.5 shadow-[var(--shadow-1)]">
          <span className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-accent text-[15px] font-semibold text-bg">{user[0]}</span>
          <span className="min-w-0 flex-1">
            <span className="block text-[15px] font-semibold text-ink">{user}</span>
            <span className="block truncate text-[12px] text-muted">{signedOut ? "Signed out in this demo only" : "Signed in with Google"}</span>
          </span>
          <button type="button" onClick={() => setSignedOut((v) => !v)} className="shrink-0 rounded-full px-2 py-1 text-[12px] font-semibold text-accent hover:bg-accent-tint">
            {signedOut ? "Sign in" : "Sign out"}
          </button>
        </section>

        <button type="button" onClick={() => leave(onNew)} className="rise mt-3 flex h-12 w-full items-center justify-center gap-2 rounded-full border-[1.5px] border-accent text-[14px] font-semibold text-accent transition-colors hover:bg-accent-tint active:scale-[0.99]" style={{ animationDelay: "40ms" }}>
